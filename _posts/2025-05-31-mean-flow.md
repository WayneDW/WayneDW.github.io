---
title: 'Mean Flow: A Brief Introduction'
...
Denote $\mathrm{x \sim p_{\text{data}}}$ and $\mathrm{\epsilon \sim p_{\text{prior}}}$. Consider the linear interpolation $\mathrm{z_t = (1 - t)x + t\epsilon}$. The flow of particles follow $\mathrm{\nu_t \equiv z_t' = \epsilon - x}$ ([link](https://www.weideng.org/posts/flow_diffusion_PDE/)) {% cite neural_ode %}, where $$\mathrm{\nu_t \equiv \nu_t(z_t \mid x)}$$ is the conditional velocity. The authors introduce the elegant concept of the **average velocity**:
...
The above equality reduces to flow matching when $\mathrm{r=t}$ and the visualization is shown below. 
...
The derivatives $\mathrm{[\partial_z u, \partial_r u, \partial_t u]}$ on the LHS can be computed via a Jacobian–vector product (JVP) along the tangent vector $\mathrm{[\nu, 0, 1]}$. Let $\mathrm{u_{\theta}}$ parameterize the mean velocity $\mathrm{u}$. The training loss becomes:
...
@misc{deng2025_mf,
  ...
  year    ={2025},
  howpublished = {\url{https://www.weideng.org/posts/mean_flow/}}
}
```
{% endraw %}
...
SeqGAN resolves these issues by modeling the generator $$\mathbf{G_{\theta}}$$ as a policy for sequence generation and using the discriminator $$\mathbf{D_{\phi}}$$ to provide the reward signal.

**Intermediate state $$\mathrm{z_{1:t}}$$**: SeqGAN performs Monte Carlo rollouts using the policy $$\mathrm{G_{\theta}}$$ to sample the remaining steps such as 
...
**Non-differentiability**: Policy gradient methods {% cite williams1992simple %} is proposed to shift from the gradient of discrete actions to the probability space using the log-derivative trick:

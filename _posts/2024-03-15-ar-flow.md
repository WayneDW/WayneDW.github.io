...
Normalizing flows {% cite normalzing_flow_jmlr %} are the pioneers in generative models through a series of invertible transformations. The main idea is to transform a random vector $\mathrm{u}$ through a transport map $\mathrm{T}$:
...
where $$\text{det}\ \mathrm{J}_{\mathrm{T}}(\mathrm{u})$$ quantifies the change of volume w.r.t. u, such that $$\text{det}\ \mathrm{J}_{\mathrm{T}}(\mathrm{u}) \approx \frac{\text{Vol}(\mathrm{d} \mathrm{x})}{\text{Vol}(\mathrm{d} \mathrm{u})}$$. 
...
$$\begin{align}
  \mathrm{T}^{-1}&=(\mathrm{T}_K \circ \cdots \circ \mathrm{T}_2 \circ \mathrm{T}_1)^{-1}=\mathrm{T}_1^{-1} \circ \mathrm{T}_2^{-1} \circ \dots \mathrm{T}_K^{-1}.\notag \\
...
$$\begin{align}
  \mathrm{L}(\phi, \psi)&=\text{KL}(\mathrm{p}^{\star}_{\mathrm{X}}(\mathrm{x})\| \mathrm{p}_{\mathrm{X}}(\mathrm{x}; \phi, \psi)) \notag \\
...
the target density $\mathrm{p}^{\star}_{\mathrm{X}}(\mathrm{x})$ (polish words ). 
...
$$\begin{align}
  \mathrm{L}(\phi, \psi)&=\text{KL}(\mathrm{p}_\mathrm{X}(\mathrm{x}; \phi, \psi) \| \mathrm{p}^{\star}_{\mathrm{X}}(\mathrm{x})) \notag \\
...
Autoregressive flows were one of the earliest flows developed. We can transform any distribution $\mathrm{p}_\mathrm{x}(\mathrm{x})$ into a uniform distribution in $(0, 1)^\mathrm{D}$ using maps with a triangular Jacobian. (polish)
...
$$\begin{align}
  \mathrm{p}_{\mathrm{X}}(\mathrm{x}) = \Pi_{i=1}^\mathrm{D} \mathrm{p}_{\mathrm{X}}(\mathrm{x}_i | \mathrm{x}_{<i}).
\end{align}$$
...
To model a sequential structure via an efficient parallelism, {% cite mask_ae %} abandoned the RNN encoder and proposed a mind-blowing idea by using an masked autoencoder.
...
The challenge is to design proper masks to satisfy the autoregressive (triangular) property. A good tutorial is provided in {% cite masked_ae_youtube_tutorial %} and a helper figure is shown below. 
...
MAF {% cite MAF %} is efficient in the conditional density estimation while the sampling stage is slow in high dimensions because we need to iterate the dimension sequentially. To help understanding the masked autoregressive flow, I simplified {% cite torch_nf %}'s code template and added some edits and comments.
...
Similar to MAF, IAF {% cite IAF %} proposed to flip the backward and forward sampling to speed up the sampling efficiency. The density estimation, however, becomes slower.

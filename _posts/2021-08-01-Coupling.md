...
$$\forall A\in \mathcal{X}, \quad \|p-q\|_{\text{TV}} = \sup_{A\in \mathcal{X}} \mathbb{P}(X\in A)- \mathbb{P}(Y\in A),\qquad\qquad\qquad\qquad\qquad\qquad$$
where $X$ and $Y$ are random variables defined on the state space $\mathbb{X}$ with measurable set $\mathcal{X}$. Note that for any measurable set $A\in\mathcal{X}$, the follow inequality always holds
...
Notably, it is clear that $D=\\{x:\ p(x)\geq q(x)\\}$. It follows that $\sup_{A\in \mathcal{X}} \mathbb{P}(X\in A)- \mathbb{P}(Y\in A)=\sup_{A\in \mathcal{X}}\nu(A)=\nu(D)=\mathbb{P}(X\in D)-\mathbb{P}(Y\in D)$. We have 
$$\mathbb{P}(X\in D)-\mathbb{P}(Y\in D)=\int_{\{x:\ p(x)\geq q(x)\}} p(x) d x - \int_{\{x:\ p(x) \geq q(x)\}} q(x) d x$$

$$\qquad\qquad\qquad\ =\int_{\{x:\ p(x)< q(x)\}} q(x) d x - \int_{\{x:\ p(x) < q(x)\}} p(x) d x.$$

<p align="center">
    <img src="/images/TS_distance.png" width="600" />
</p>
Summing up the above equations and combining $|a-b|=a+b-2 a\wedge b$, we can obtain the area between the two pdf curves [1]

$$\|p-q\|_{\text{TV}}=\frac{1}{2}\int |p(x)- q(x)| d x=1-\int \min\{p(x), q(x)\} d x.$$

In the end, we can summarize the different formulations of total variation
$$\ \ \|p-q\|_{\text{TV}}=\sup_{A\in \mathcal{X}} \mathbb{P}(X\in A)- \mathbb{P}(Y\in A)\qquad\qquad\qquad\qquad\qquad\qquad$$
$$\qquad\qquad\quad =\frac{1}{2}\int |p(x)- q(x)| d x\qquad\qquad\qquad\qquad\qquad\qquad\qquad$$
$$\qquad\qquad\ \ \ =1-\int \min\{p(x), q(x)\} d x\qquad\qquad\qquad\qquad\qquad\qquad$$
$$\qquad\qquad\quad=\inf_{(X,Y)\in \text{coupling}(p, q)} \mathbb{P}(X\neq Y).\qquad\qquad\qquad\qquad\qquad\qquad$$
...
$$\|\mathcal{L}(X_t)-\mathcal{L}(Y_t)\| \leq \mathbb{P}(\tau>t),$$
...
[1] Pierre E. Jacob. Couplings and Monte Carlo. Slides.

[2] Xiang Cheng, Niladri S. Chatterji, Peter L. Bartlett, Michael I. Jordan. Underdamped Langevin MCMC: A non-asymptotic analysis. COLT 2018.

[3] Arnak S. Dalalyan. A series of Langevin Monte Carlo Methods.

[4] O. Mangoubi and A. Smith. Rapid mixing of Hamiltonian Monte Carlo on strongly log-concave distributions. arXiv preprint arXiv:1708.07114, 2017.

<<<<<<< HEAD

[5] Torgny Lindvall and L. C. G. Rogers. Coupling of Multidimensional Diffusions by Reflection. Annals Probability. 14(3): 860-872, 1986

[6] Mu-Fa Chen and Shao-Fu Li. Coupling Methods for Multidimensional Diffusion Processes. Annals of Probability. 17(1): 151-177, 1989

[7] N. Bou-Rabee, A. Eberle, and R. Zimmer. Coupling and convergence for Hamiltonian Monte Carlo. Annals of Applied Probability, 2020, Vol. 30, No. 3, 1209-1250.

[8] Alain Durmus and Eric Moulines. High-dimensional Bayesian inference via the Unadjusted Langevin Algorithm. Bernoulli 25(4A):2854-2882, 2019.
=======
[5] T. Lindvall and L. C. G. Rogers. Coupling of Multidimensional Diffusions by Reflection. Annals of Probability 14(3): 860-872, 1986

[6] M. Chen and S. Li. Coupling Methods for Multidimensional Diffusion Processes. Annals of Probability. 17(1): 151-177, 1989

[7] N. Bou-Rabee, A. Eberle, and R. Zimmer. Coupling and convergence for Hamiltonian Monte Carlo. Annals of Applied Probability 30(3): 1209-1250. 2020

[8] A. Durmus and E. Moulines. High-dimensional Bayesian inference via the Unadjusted Langevin Algorithm. Bernoulli 25(4A):2854-2882, 2019.

[9] A. Eberle. Reflection Coupling and Wasserstein Contractivity without Convexity. Comptes Rendus Mathematique 349 (19-20), 1101-1104, 2011.

[10] A. Eberle. Reflection couplings and contraction rates for diffusions. Prob. Theo. Related. Fields. 166, pages 851–886, 2016
>>>>>>> ab793e066f1fee95b9a08d783e6df7248197fdc4

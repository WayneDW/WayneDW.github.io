...
Consider Itô's formula for the sequence of $\frac{1}{2}  \|\| \theta_t - \beta_t \|\| ^2$ and apply a synchronous coupling between the two processes to cancel the noise terms, we have
$\frac{1}{2} d  \|\| \theta_t - \beta_t \|\|^2 = \langle \theta_t - \beta_t, d \theta_t - d \beta_t \rangle + \mathrm{Tr}[ d^2 \theta_t - d^2 \beta_t ]$
$\qquad=\langle \theta_t - \beta_t, (\nabla f(\beta_t) -\nabla\widetilde  f(\theta_{\eta\lfloor\frac{t}{\eta} \rfloor}) ) d t + \sqrt{2\tau} ( d W_t - d W_t ) \rangle + 2\tau \mathrm{Tr}[ d^2 W_t - d^2 W_t ].$

...
\begin{align}
\frac{1}{2} d  \|\| \theta_t - \beta_t \|\|^2 = \langle \theta_t - \beta_t, (\nabla f(\beta_t) -\nabla\widetilde  f(\theta_{\eta\lfloor\frac{t}{\eta} \rfloor}) ) d t  \rangle \\
\end{align}

where a synchronous coupling between the two processes has been applied to remove the term $\sqrt{2\tau}( d W_t -  d W_t ) }$ and $2\tau \mathrm{Tr}[ d^2 W_t - d^2 W_t ]$.
...
&= \langle \theta_t - \beta_t, (\nabla f(\beta_t) -\nabla\widetilde  f(\theta_{\eta\lfloor\frac{t}{\eta} \rfloor}) ) d t + \sqrt{2\tau}( d \hat{W}_t -  d W^*_t ) } \rangle + 2\tau \mathrm{Tr}[ d^2 \hat{W}_t - d^2 W^*_t ].
\end{split}
\end{equation}

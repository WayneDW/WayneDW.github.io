...
where $\tau$ is a random variable that enables $X_t=Y_t$ and is also known as ''meeting time''. The equality can be achieved given the maximal coupling but the contruction may differ in different problems.

In addition, meeting exacting sometimes is quite restricted; it is enough to consider close enough chains.

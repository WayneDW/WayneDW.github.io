...
Given the particle $$\mathrm{X_0^i}\sim \mathrm{P_X}$$ and the corresponding observation $$\mathrm{Y_0^i \sim P_{Y\\|X}(\cdot \\| X_0^i)}$$ for $\mathrm{i=1,2,\cdots, N}$, $$\mathrm{\{X_0^i, Y_0^i\}_{i=1}^N}$$ provides samples from the joint distribution $\mathrm{P_{XY}}$. We can define an unbiased cost function \eqref{cost_func} as follows:

...
Given the particle $$\mathrm{X_0^i}\sim \mathrm{P_X}$$ and observation $$\mathrm{Y_0^i \sim P_{Y\\|X}(\cdot \\| X_0^i)}$$ for $\mathrm{i=1,2,\cdots, N}$. We can re-define the an unbiased cost function \eqref{cost_func} as follows:

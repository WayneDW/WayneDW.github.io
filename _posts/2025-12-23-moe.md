...
**Mixture of experts (MoE)** scales model parameters and improves generation quality without increasing **per-token FLOPs**. Despite the name, MoE doesn't mean assembling human-like experts (e.g., finance or medicine). Instead, it replaces a big feedforward network (FFN) {% cite shazeer2017outrageously %} {% cite fedus2022switch %} {% cite deepseekv3_2024 %} {% cite openai_gptoss_github %} with many smaller blocks, where only a subset is activated for each token.
...
class MLPBlock(nn.Module):
    def __init__(self, hidden_size, intermediate_size, num_experts, experts_per_token, swiglu_limit):
        ...
        self.norm = RMSNorm(hidden_size) # no normalization: affect <0.2% FLOP but >20% runtime; no bias: resembles transformers more stable
...
Alternative approaches include formulating routing as an optimal transport problem to achieve balanced expert assignment, or introducing stochastic perturbations {% cite shazeer2017outrageously %} to the gating mechanism to improve load balance and training stability.

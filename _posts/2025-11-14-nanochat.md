...
## scripts: Training, SFT, RL, and Eval Pipeline
...
Pretraining focuses on next-token prediction, whereas SFT uses labeled instruction–response pairs to teach models how to follow user prompts, such as producing exact answers in GSM8K or code that passes unit tests in MBPP.
...
| **Data** | `Massive corpus` | `SmolTalk, GSM8K, ...` | `Instruction–response` | `Prompts + model rollouts` |
| **Objective** | `Next-token` | `Next-token` | `Assistant-only supervised` | `"GRPO"` |
| **Supervision** | `Unlabeled text` | `Multitask tokens` | `Instruction → assistant` | `Reward from generated answer` |
| **Target** | `All tokens` | `All tokens` | `Assistant-only` | `Generated answer tokens` |
| **Sampling** | ✗ | ✗ | ✗ | ✓ |
| **Rewards** | ✗ | ✗ | ✗ | ✓ |
| **Sequence** | `Fixed length` | `Fixed` | `Variable` | `Variable rollouts` |
| **Masking** | `None` | `None` | `Mask non-assistant` | `Mask prompt & padding` |
| **What It Teaches** | `Knowledge, reasoning` | `Math, QA, skills` | `Instruction following, chat` | `Correctness, logic, math accuracy` |

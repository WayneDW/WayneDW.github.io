...
subtitle: Sparse routing for scalable Transformer capacity
date: 2025-12-22
permalink: /posts/moe/
...
**Mixture of experts (MoE)** is an effective way to scale model parameters and enhance generation quality without increasing **per-token FLOPs** {% cite fedus2022switch %} {% cite deepseekv3_2024 %}. Despite the name, MoE doesn't mean assembling human-like experts (e.g., finance or medicine). Instead, it replaces a big transformer or MLP block with many smaller blocks, only a subset of which is actived for each token. 
...
almost all do: Token → choose experts
